...
void main() {
    ...
    char buffer[20];
    pipe(fd1);            // Creamos la tuberia 1
    pipe(fd2);            // Creamos la tuberia 2
    ...
    if(pidNieto == 0) { // Estamos en el Nieto
        ...
        write(fd1[1], "Saludo del nieto...", sizeof(buffer)); // Enviamos un mensaje al HIJO
    }
    else {
        if (pidHijo == 0) { // Estamos en el Hijo
            ...
            read(fd1[0], buffer, sizeof(buffer)); // Leemos lo que hay en la pipe que ha enviado el NIETO
            printf("\tEl HIJO recibe un mensaje de su hijo: %s\n", buffer); // Lo visualizamos
            printf("\tEl HIJO envia un mensaje al ABUELO......\n");
            write(fd2[1], "Saludo del hijo..", sizeof(buffer));   // Enviamos un mensaje al ABUELO
        }
            ...
            read(fd2[0], buffer, sizeof(buffer)); // Lees lo que hay en la pipe que ha enviado el HIJO
